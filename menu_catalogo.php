<div class="container-fluid">
			<div id="bloque1" class="cd-header cd-morph-dropdown">
				<a href="<?php echo get_site_url(); ?>/" ><figure class="logo_pc"><img src="<?php echo get_template_directory_uri(); ?>/imagenes/iconos/LOGO.png" alt="Logo a1security"></figure></a>
				<a href="#0" class="nav-trigger">Open Nav<span aria-hidden="true"></span></a>
				<nav class="main-nav menu">
					<ul id="menupc">						
						<li class="links  sub-menu-parent active " >
							<a href="<?php echo get_site_url(); ?>" class="menu__link letra17pt-pc">HOME&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;</a>
						</li>					
						<li class="links  sub-menu-parent active " >
							<a href="tel:+19178283434" class="menu__link letra17pt-pc">CALL (917) 828-3434&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;</a>
						</li>
						<li class="links  sub-menu-parent active " data-content="aboutus">
							<a href="<?php echo get_site_url(); ?>/?page_id=71" class="menu__link letra17pt-pc">ABOUT US &nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;</a>
						</li>
						<li class="links  sub-menu-parent " data-content="clients">
							<a href="<?php echo get_site_url(); ?>/?page_id=93" class="menu__link letra17pt-pc">CLIENTS &nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp; </a>
						</li>
						<li class="links  sub-menu-parent " data-content="testimonials">
							<a href="<?php echo get_site_url(); ?>/?page_id=143" class="menu__link letra17pt-pc">TESTIMONIALS</a>
						</li>
					</ul>
				</nav>

				<div class="morph-dropdown-wrapper">
					<!-- Accordion with No Space example -->
					<div class="panel-group pmd-accordion pmd-accordion-nospace tablet celular" id="accordion5" role="tablist" aria-multiselectable="true" > 
						<div class="panel panel-default"> 
							<a href="<?php echo get_site_url(); ?>/"><img src="<?php echo get_template_directory_uri(); ?>/imagenes/iconos/LOGO.png" alt="Logo Dra Milena" class="logo_movil tablet celular"></a> 
						</div>
						<div class="panel panel-default"> 
							<div class="panel-heading" role="tab" id="headingOne">
								<h4 class="panel-title"><a href="<?php echo get_site_url(); ?>/"> Home</a></h4>
							</div>
						</div>
						<div class="panel panel-default"> 
							<div class="panel-heading" role="tab" id="headingOne">
								<h4 class="panel-title"><a href="tel:+19178283434" onclick="return gtag_report_conversion('tel:917-828-3434');"> Call (917) 828-3434</a></h4>
							</div>
						</div>
						<div class="panel panel-default"> 
							<div class="panel-heading" role="tab" id="headingOne">
								<h4 class="panel-title"><a href="<?php echo get_site_url(); ?>/?page_id=71"> About us</a></h4>
							</div>
						</div>
						<div class="panel panel-default"> 
							<div class="panel-heading" role="tab" id="headingOne">
								<h4 class="panel-title"><a href="<?php echo get_site_url(); ?>/?page_id=93"> Clients</a></h4>
							</div>
						</div>
						<div class="panel panel-default"> 
							<div class="panel-heading" role="tab" id="headingOne">
								<h4 class="panel-title"><a href="<?php echo get_site_url(); ?>/?page_id=143"> Testimonials</a></h4>
							</div>
						</div>
					</div> <!-- Accordion with No Space example end -->
					</div> <!-- Accordion with No Space example end -->
				</div>	
					<!-------------------------------------------------------------------------------------------------------------------------------->